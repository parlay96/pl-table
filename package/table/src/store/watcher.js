...
import merge from 'pl-table/src/utils/merge';
...
export default Vue.extend({
  data() {
    return {
      states: {
        ...
        newisAllSelected: false // 大数据渲染模式是否为非全选状态，config.js 只针对对el-checkbox 的indeterminate属性
      }
    };
  },
  ...
  methods: {
    ...
    isSelected(row) {
      // 修改源码: 全选按钮的条件判断
      if (this.states.isAllSelected && this.states.useVirtual && this.states.bigDataCheckbox) {
        const { selection = [] } = this.states;
        if (row['plChoose'] === false) {
          return false;
        } else {
    ...
    toggleRowSelection(row, selected, emitChange = true) {
      ...
      const changed = toggleRowStatus(this.states.selection, row, selected, this);
      if (changed) {
        const newSelection = (this.states.selection || []).slice();
        // 修改源码: 如我我当前是大数据并且我当前是全选状态下，
        // 因为改了源码的复选框勾选方式，所以需要对newSelection数据进行筛选，才传值出去
        if (this.states.useVirtual && this.states.isAllSelected && this.states.bigDataCheckbox) {
          // 只有当我plChoose是选中的情况下才传值出去
          const data = newSelection.filter(item => item.plChoose)
