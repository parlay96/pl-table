import Vue from 'vue';
import { hasClass, addClass, removeClass } from 'pl-table/src/utils/dom';
import ElCheckbox from 'pl-table/package/checkbox';
              ...
              >
                {
                  columns.map((column, cellIndex) => (<th
                    ...
                    key={ column.id }>
                   {/*修改源码 添加拖动图标, 当我开启了列不可拖动显示线条并且不是掉落元素，或者可拖动列的情况 就触发下面渲染*/}
                    {
                      this.$parent.headerDragShow && this.$parent.headerDragStyle && !column.dropAction || column.resizable && this.$parent.headerDragStyle
                  ...
                  {/*修改源码 当列的宽度小于内容了，不要换行显示，超出就省略符合 所以添加了个类pl-table-beyond*/}
                    <div class={[
                       'cell',
                       this.$parent.useVirtual ? 'pl-table-beyond' : '',
